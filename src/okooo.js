...
async function getIdFromDb() {
  ...
  const [rows, fields] = await conn.execute(
    "SELECT max(id)+1 as id FROM `t_match`"
  );
  conn.release();
...
const ODDS_SQL =
  "insert into t_match_odds(matchId,companyId,s,p,f,h,pan,a) values";
const MATCH_SQL =
...
const oddsSql =
  "select m.id  from t_match m left join t_match_odds o on m.id = o.matchId where o.matchId is null and m.id limit 100";
  ...
  if (g_cache_data.matches.length < 10 && match) {
    ...
    if (sql_values.length > 0) {
      sql =
        MATCH_SQL +
        sql_values.join(",") +
        "  ON DUPLICATE KEY UPDATE `version`=`version` + 1 , `fullscore`=values(`fullscore`),`goalscore`=values(`goalscore`),`result`=values(`result`),`halfscore`=values(`halfscore`) ";
      try {
        const [rows, fields] = await conn.execute(sql);
...
async function getMatchCallback(d) {
  var data = await hook_page.evaluate(
    (d, id) => {
  ...
  if (minId < maxId) {
    console.log(
      new Date(),
      "id=" + minId + "已经获取完成，还剩 " + (maxId - minId) + " 个"
    );
    minId += 1;
    ...
    console.log(
      new Date(),
      "当前id=" + minId + ",已经到了最大id=" + maxId + "，结束程序"
    );
    await doFinish();
  }
}
